...
imline = args[1] 
pheno = args[2] 
# inpath <- "/Users/pveltsos/Desktop/qtl/"
...
# pheno <- "N0405" # D0488 D0553 D0319 D0369
# Remove missing data
...
lod.all.effect.table <- merge(lodTable, effectTable, by.x="markers", by.y="markers", all=T )
...
colnames(simple.lod.all.effect.table) <- c('pheno', 'snp', 'lg', 'cm', 'lod', 'p', 'a', 'd', 'se.a', 'se.d', 'threshold')
...
# marker <- find.marker(mapthis2, 4, 16.641)
