...
kdata <- read.table(file.path(datapath, paste(GENE, "_", DISTANCE, "_temp_control_positive.txt", sep="")), header=F)
...
write.table(kdata3, file=file.path(datapath, paste(GENE, DISTANCE, "temp_control_positive2.txt", sep="_")), quote=F, row.names=F, sep="\t")
